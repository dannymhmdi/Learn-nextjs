    ...
    <>
      ...
      {links.map(link => {
        return (
            ...
            key={link.id}
            href={link.route}
            className={`bg-slate-800 text-white w-24 text-center rounded-lg p-3 ml-2 mt-2 inline-block ${pathName === 'https://learn-nextjs-nine-ruddy.vercel.app/about-us' ? 'bg-red-500': ''}`}
          >
            {link.name}
          </Link>
        )
